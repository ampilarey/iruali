                    ...
                    <table class="w-full text-sm border mb-2">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><input type="number" name="island_stock[{{ $island->id }}][stock_quantity]" min="0" class="w-24 border rounded p-1" value="0"></td>
                                <td><input type="number" name="island_stock[{{ $island->id }}][reorder_point]" min="0" class="w-24 border rounded p-1" value="10"></td>
                                <td class="text-center"><input type="checkbox" name="island_stock[{{ $island->id }}][is_active]" value="1"></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createVariantRow(index = null, data = {}) {
        ...
        row.innerHTML = `
            <td><input type="text" name="variants[
                ${index ?? 'IDX'}][name][en]" class="w-32 border rounded p-1" value="${data.name?.en ?? ''}"></td>
            <td><input type="text" name="variants[
                ${index ?? 'IDX'}][name][dv]" class="w-32 border rounded p-1" value="${data.name?.dv ?? ''}"></td>
            <td><input type="text" name="variants[
                ${index ?? 'IDX'}][type]" class="w-24 border rounded p-1" value="${data.type ?? ''}"></td>
            <td><input type="text" name="variants[
                ${index ?? 'IDX'}][sku]" class="w-24 border rounded p-1" value="${data.sku ?? ''}"></td>
            <td><input type="number" step="0.01" name="variants[
                ${index ?? 'IDX'}][price_adjustment]" class="w-20 border rounded p-1" value="${data.price_adjustment ?? 0}"></td>
            <td><input type="number" name="variants[
                ${index ?? 'IDX'}][stock_quantity]" class="w-16 border rounded p-1" value="${data.stock_quantity ?? 0}"></td>
            <td><input type="file" name="variants[
                ${index ?? 'IDX'}][image]" class="w-32"></td>
            <td class="text-center"><input type="checkbox" name="variants[
                ${index ?? 'IDX'}][is_active]" value="1" ${data.is_active ? 'checked' : ''}></td>
