...
@section('title', $product->name . ' - Iru E-commerce')
                ...
                <div class="mb-6">
                    @if($product->is_on_sale)
                        <div class="flex items-center space-x-4">
                            <span class="text-3xl font-bold text-primary-600">ރ{{ number_format($product->final_price, 2) }}</span>
                            <span class="text-xl text-gray-500 line-through">ރ{{ number_format($product->price, 2) }}</span>
                            <span class="bg-red-500 text-white px-3 py-1 rounded-full text-sm font-semibold">
                                -{{ $product->discount_percentage }}% OFF
                            </span>
                        </div>
                    @else
                        <span class="text-3xl font-bold text-primary-600">ރ{{ number_format($product->price, 2) }}</span>
                    @endif
                </div>
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            @if($relatedProduct->is_on_sale)
                                <span class="text-lg font-bold text-primary-600">ރ{{ number_format($relatedProduct->final_price, 2) }}</span>
                                <span class="text-sm text-gray-500 line-through">ރ{{ number_format($relatedProduct->price, 2) }}</span>
                            @else
                                <span class="text-lg font-bold text-primary-600">ރ{{ number_format($relatedProduct->price, 2) }}</span>
                            @endif
                        </div>
